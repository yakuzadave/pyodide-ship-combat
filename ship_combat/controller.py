...
class PhaseSnapshot:
    ...
    turn: int
    phase_name: str
    fleets: Mapping[str, FleetSnapshot] = field(
...
class BattleSnapshotController:
    ...
    def record_phase(
        ...
        fleet_snapshots = {
            name: FleetSnapshot.from_fleet(name, list(fleet)) for name, fleet in fleets.items()
        }
        phase_snapshot = PhaseSnapshot(
            turn=self._current_turn.turn,
            phase_name=phase_name,
            fleets=MappingProxyType(fleet_snapshots),
